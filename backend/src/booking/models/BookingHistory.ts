...
const schema = new Schema({ bookingId: { type: Types.ObjectId, ref: 'Booking', required: true, index: true }, action: { type: String, required: true }, status: { type: String, required: true }, actorId: { type: Types.ObjectId, ref: 'User' }, note: String, snapshot: Schema.Types.Mixed, occurredAt: { type: Date, required: true, default: Date.now } }, { versionKey: false });
