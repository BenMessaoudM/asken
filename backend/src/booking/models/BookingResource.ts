...
const localized = new Schema({ en: { type: String, required: true }, sv: { type: String, required: true } }, { _id: false });
...
const schema = new Schema({
  ...
  name: { type: localized, required: true }, description: { type: localized, required: true }, location: { type: localized, required: true }, rules: { type: localized, required: true },
  ...
  openingHours: { type: [opening], default: [] }, blackoutPeriods: { type: [blackout], default: [] }, schemaVersion: { type: Number, default: 1 },
}, { timestamps: true });
