...
const sampleEvent: ManagedEvent = { id: '507f1f77bcf86cd799439042', contentId: '507f1f77bcf86cd799439043', slug: 'welcome-event', publicationStatus: 'draft', temporalStatus: 'upcoming', version: 1, translations: { en: { title: 'Welcome event', description: 'Join us', organizer: 'ASK', location: 'Cor' }, sv: { title: 'Välkomstevenemang', description: 'Kom med', organizer: 'ASK', location: 'Cor' } }, startAt: new Date('2030-01-01T10:00:00Z'), endAt: new Date('2030-01-01T12:00:00Z'), category: eventCategory, eventStatus: 'scheduled', featured: true, eventCollaborations: [], createdAt: new Date(), updatedAt: new Date() };
const publicEvent: PublicEvent = { id: sampleEvent.id, slug: sampleEvent.slug, ...sampleEvent.translations.sv, startAt: sampleEvent.startAt, endAt: sampleEvent.endAt, eventStatus: 'scheduled', temporalStatus: 'upcoming', category: { slug: eventCategory.slug, label: eventCategory.labels.sv }, featured: true, locale: 'sv', eventCollaborations: [] };
...
describe('API foundation and identity integration', () => {
  ...
  it('manages event categories and event lifecycle', async () => { const service=createEventService(),app=buildApp(createIdentityService(),createCmsService(),createNewsService(),service),session=await login(app),cookie=cookies(session);const eventCollaborations=[{collaborationId:'507f1f77bcf86cd799439091',role:'sponsor',displayOrder:1,visible:true,note:{sv:'Tack',en:'Thanks'}}];const payload={translations:sampleEvent.translations,startAt:'2030-01-01T10:00:00.000Z',endAt:'2030-01-01T12:00:00.000Z',categoryId:eventCategory.id,eventStatus:'scheduled',featured:true,eventCollaborations};expect((await request(app).post('/api/v1/admin/events').set('Cookie',cookie).send(payload)).status).toBe(201);expect(service.createEvent).toHaveBeenCalledWith(expect.objectContaining({eventCollaborations:expect.arrayContaining([expect.objectContaining({role:'sponsor'})])}),expect.anything(),expect.anything());expect((await request(app).put(`/api/v1/admin/events/${sampleEvent.id}`).set('Cookie',cookie).send({...payload,expectedVersion:1})).status).toBe(200);expect((await request(app).post(`/api/v1/admin/events/${sampleEvent.id}/publish`).set('Cookie',cookie).send({expectedVersion:1})).body.data.event.publicationStatus).toBe('published');expect((await request(app).patch(`/api/v1/admin/events/${sampleEvent.id}/featured`).set('Cookie',cookie).send({featured:false})).body.data.event.featured).toBe(false);expect((await request(app).post('/api/v1/admin/events/categories').set('Cookie',cookie).send({labels:{en:'Culture',sv:'Kultur'}})).status).toBe(201);expect((await request(app).delete(`/api/v1/admin/events/${sampleEvent.id}`).set('Cookie',cookie)).status).toBe(204); });
  it('validates event collaboration roles and duplicates', async () => { const app=buildApp(),session=await login(app),cookie=cookies(session);const base={translations:sampleEvent.translations,startAt:'2030-01-01T10:00:00.000Z',endAt:'2030-01-01T12:00:00.000Z',categoryId:eventCategory.id,eventStatus:'scheduled',featured:true};expect((await request(app).post('/api/v1/admin/events').set('Cookie',cookie).send({...base,eventCollaborations:[{collaborationId:'507f1f77bcf86cd799439091',role:'bad_role',displayOrder:0,visible:true}]})).status).toBe(400);expect((await request(app).post('/api/v1/admin/events').set('Cookie',cookie).send({...base,eventCollaborations:[{collaborationId:'507f1f77bcf86cd799439091',role:'sponsor',displayOrder:0,visible:true},{collaborationId:'507f1f77bcf86cd799439091',role:'sponsor',displayOrder:1,visible:true}]})).status).toBe(400); });
