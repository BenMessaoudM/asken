...
const collaboration: Collaboration = { id: '507f1f77bcf86cd799439091', name: 'ASK Partner', slug: 'ask-partner', type: 'strategic_partner', description: { sv: 'Svensk text', en: '' }, shortDescription: { sv: 'Kort', en: '' }, logoAltText: { sv: '', en: '' }, contactPerson: 'Kontakt', phone: '040 123 4567', socialLinks: { youtube: 'https://youtube.example' }, officeAtCor: true, officeLocation: 'Cor-huset', officeHours: { sv: 'Mån 10.00-12.00', en: '' }, publicContactInfo: { sv: 'Kontakta oss', en: '' }, active: true, visible: true, featured: true, displayOrder: 1, tags: { sv: 'partner', en: '' }, internalNotes: 'Admin-only', createdAt: new Date(), updatedAt: new Date() };
...
const publicCollaboration = { id: collaboration.id, name: collaboration.name, slug: collaboration.slug, type: collaboration.type, typeLabel: 'Partner', description: 'Svensk text', shortDescription: 'Kort', logoAltText: '', contactPerson: 'Kontakt', phone: '040 123 4567', socialLinks: collaboration.socialLinks, officeAtCor: true, officeLocation: 'Cor-huset', officeHours: 'Mån 10.00-12.00', publicContactInfo: 'Kontakta oss', featured: true, displayOrder: 1, tags: 'partner', updatedAt: collaboration.updatedAt };
function service(): CollaborationService { return { listPublic: jest.fn(async (filters) => filters.type === 'student_nation' ? [] : [publicCollaboration]), getPublicBySlug: jest.fn(async () => publicCollaboration), listTypes: jest.fn(async () => [{ type: 'strategic_partner', label: 'Partner' }, { type: 'alumni_association', label: 'Alumnförening' }]), getPublicSettings: jest.fn(async () => ({ intro: 'Intro', featuredIntro: 'Utvalt', visible: true, updatedAt: new Date() })), listAdmin: jest.fn(async () => [collaboration]), create: jest.fn(async (input) => ({ ...collaboration, ...input })), update: jest.fn(async (_id, input) => ({ ...collaboration, ...input })), deactivate: jest.fn(async () => undefined), getSettings: jest.fn(async () => ({ id: 'settings', intro: { sv: 'Intro', en: 'Intro' }, featuredIntro: { sv: 'Utvalt', en: 'Featured' }, visible: true, updatedAt: new Date() })), updateSettings: jest.fn(async (input) => ({ id: 'settings', ...input, updatedAt: new Date() })) } as unknown as CollaborationService; }
...
describe('collaboration routes', () => {
  it('serves public list, detail, types, settings and forwards filters', async () => {
    ...
    expect((await request(application).get('/api/v1/collaborations?lang=sv')).body.data.collaborations[0].internalNotes).toBeUndefined();
    ...
    await request(application).get('/api/v1/collaborations?featured=true&officeAtCor=true&tag=partner');
    expect(collaborationsService.listPublic).toHaveBeenCalledWith(expect.objectContaining({ featured: true, officeAtCor: true, tag: 'partner' }), 'sv');
