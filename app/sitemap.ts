...
const base="https://ask-student-union.musse97.chatgpt.site";
...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
  ...
  try{
    const items=await getDb().select({type:contentItems.type,slug:contentItems.slug,updatedAt:contentItems.updatedAt}).from(contentItems).where(eq(contentItems.status,"published"));
