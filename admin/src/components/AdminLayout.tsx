    ...
    <div className="min-h-screen bg-[#F7F3F5] text-ask-ink transition-colors dark:bg-[#17161C] dark:text-[#F8F4F7]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-ask-ink text-white shadow-2xl transition-transform duration-200 dark:border-r dark:border-white/10 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-5" aria-label="Dashboard modules">
          <p className="px-3 pb-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/40">Workspace</p>
        ...
        <div className="border-t border-white/10 p-4">
          ...
          <NavLink to="/change-password" className="mb-2 block w-full rounded-xl px-3 py-2 text-center text-sm font-medium text-white/65 transition hover:bg-white/10 hover:text-white">Account security</NavLink>
          <button onClick={() => void handleLogout()} className="w-full rounded-xl border border-white/15 px-3 py-2 text-sm font-medium text-white/75 transition hover:border-white/30 hover:bg-white/10 hover:text-white">Sign out</button>
        </div>
      </aside>
