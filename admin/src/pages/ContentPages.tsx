    ...
    <section className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-2xl border border-black/5 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
        <table className="min-w-full">
          ...
          <tbody>{contents.map((content) => <tr key={content.id} className="border-b border-black/5 last:border-0 dark:border-white/10"><td className="p-4"><p className="font-semibold">{content.title}</p><p className="text-sm text-gray-500 dark:text-white/40">/{content.slug}</p></td><td className="p-4 text-sm font-medium">{contentTypeLabels[content.contentType]}</td><td className="p-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${content.status === 'published' ? 'bg-green-100 text-green-800 dark:bg-green-500/15 dark:text-green-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-500/15 dark:text-amber-300'}`}>{content.status}</span></td><td className="p-4">v{content.version}</td><td className="p-4">{content.sectionCount}</td><td className="p-4 text-sm">{formatDate(content.updatedAt)}</td><td className="p-4"><div className="flex flex-wrap gap-2"><Link to={`/content/${content.id}`} className="rounded-lg border px-3 py-1.5 text-sm">{canWrite ? 'Edit' : 'View'}</Link>{canWrite && content.status === 'draft' && <button onClick={() => void publish(content)} className="rounded-lg bg-ask-600 px-3 py-1.5 text-sm text-white">Publish</button>}{canWrite && <button onClick={() => void remove(content)} className="rounded-lg border border-red-200 px-3 py-1.5 text-sm text-red-700 dark:border-red-500/30 dark:text-red-300">Delete</button>}</div></td></tr>)}</tbody>
        </table>
