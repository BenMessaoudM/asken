...
| Admin Backoffice | Partial | 58% | Real admin pages exist for auth/session restoration, users, roles, content, News, Events, booking, resources, pricing, Collaborations, and settings. Cor Activities and general Settings are still placeholders. |
...
| Collaborations | Foundation complete | 52% | Backend model/API, admin management, public list/detail pages, settings, permissions, and migration `017-collaborations` exist. Future Events, Live at Cor, Membership, homepage, Media Library, and accounting/CRM integrations are out of scope. |
