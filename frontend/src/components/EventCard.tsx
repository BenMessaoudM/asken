  ...
  return <article className={dark ? 'group overflow-hidden rounded-3xl border border-white/10 bg-white/[0.06] transition hover:-translate-y-1 hover:border-ask-400/50' : 'ask-card ask-card-hover group overflow-hidden'}>
    ...
    <div className="p-6">
      <div className="flex items-start gap-4">
        <div className="grid w-14 shrink-0 place-items-center rounded-2xl bg-ask-gold px-2 py-3 text-center text-ask-ink"><span className="text-xs font-black uppercase">{formatDate(event.startAt).slice(3)}</span><span className="text-2xl font-black leading-none">{formatDate(event.startAt).slice(0,2)}</span></div>
      ...
      <p className={`mt-5 text-sm ${dark ? 'text-white/60' : 'text-black/55 dark:text-white/55'}`}>{formatDateTime(event.startAt)} · {event.location}</p>
    </div>
  </article>
}
