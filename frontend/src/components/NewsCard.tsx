...
export default function NewsCard({ article, locale, readMore }: { article: PublicNewsArticle; locale: string; readMore: string }) {
  return <article className="ask-card ask-card-hover group overflow-hidden">
    ...
    <div className="p-6">
      <div className="flex items-center justify-between gap-3 text-xs font-bold uppercase tracking-wider text-ask-600 dark:text-ask-400"><span>{article.categories[0]?.label || 'ASK'}</span><time dateTime={article.publishedAt}>{new Date(article.publishedAt).toLocaleDateString(locale)}</time></div>
